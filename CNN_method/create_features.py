"""
This files creates the X and y features in joblib to be used by the predictive
models.
"""
...
class CreateFeatures:
    ...
    def features_creator(path, save_dir) -> str:
        ...
        for subdir, dirs, files in os.walk(path):
            for file in files:
                try:
                    # Load librosa array, obtain mfcss, store the file
                    # and the mcss information in a new array
                    ...
                    # The instruction below converts the labels (from 1 to 8)
                    # to a series from 0 to 7
                    # This is because our predictor needs to start from 0
                    # otherwise it will try to predict also 0.
                    file = int(file[7:8]) - 1
                    arr = mfccs, file
                    lst.append(arr)
        ...
        print(
            "--- Data loaded. Loading time: %s seconds ---"
            % (time.time() - start_time),
        )

        # Creating X and y: zip makes a list of all the first elements,
        # and a list of all the second elements.
        X, y = zip(*lst)
